...
- Optional Google/Microsoft integrations (user-initiated; Google Limited Use). Desktop ready-to-send Gmail replies: periodic inbox checks write a reply for review in Tasks → Inbox; send only on explicit confirm (never auto-send).
